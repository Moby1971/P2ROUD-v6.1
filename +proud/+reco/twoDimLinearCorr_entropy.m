...
% EPI Nyquist ghost correction with a 2D linear-phase model
%
%   [epiCorr, phasePara] = proud.reco.twoDimLinearCorr_entropy(epiRaw, nShot)
%
%   epiRaw      4D k-space, [kx ky slice coil]
%   nShot       number of EPI shots (interleaves)
%   epiCorr     corrected k-space, same size as epiRaw
%   phasePara   nSlice x 3, the fitted [constant, linear_x, linear_y] per slice
%
% Extends the usual 1D model with a linear phase term along the phase-encoding
% direction as well, which is what corrects oblique ghosting. The phase model is
%
%   phi(x,y) = constant + linear_x * x + linear_y * y
%
% and the three parameters are chosen per slice by minimising the entropy of
% the resulting image, so no separate reference scan is needed.
%
% Gustav Strijkers
